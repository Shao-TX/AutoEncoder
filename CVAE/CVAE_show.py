...
# %%
with torch.no_grad():
    test_all = next(iter(test_loader))
    test_data = test_all[0].to(device).view(-1, IMG_SIZE)
    test_label = test_all[1].to(device)

    # Number
    print("Number : ", test_label)
    ...
    # Reconstrate Image
    outputs, _, _ = model(test_data, test_label)
...
def generator():
    ...
    print(num)
    latent = torch.randn(32).unsqueeze(0) # [32] -> [1, 32]
    output = model.decoder(latent.to(device), num.to(device))
